...
//Code Here
let postTaxPrices = prices.map(function(element, index) {
    return element * 1.07
}); 
...
//Code Here
let totalPopulation = populations.reduce(function(runningTotal, curElement, curIndex) {
...
// Code Here
var myStrongest = monstersInYourPocket.filter(function(element, index){
  //  monstersInYourPocket[element].CP > 200
...
let orderTotals = orders.map(function(element, index){
  var taxTotal = element.price * element.tax
  let total = taxTotal + element.price 
  return total
})
...
let bobsTotal = purchases.filter(function(element, index){
  element.owner["Bob"] 
})
